...
class HalfCheetahMassEnv(HalfCheetahEnv):
    ...
    def sample_tasks(self, num_tasks):
        if self.env_type == 'test':
            masses_low = np.random.uniform(0.1, 0.5, size=(num_tasks // 2,))
            masses_high = np.random.uniform(2.9, 3.0, size=(num_tasks - num_tasks // 2,))
            ...
            masses = np.random.uniform(1.5, 2.5, size=(num_tasks,))
            tasks = [{'mass': mass} for mass in masses]
        return tasks
